...
extern void CAN_transmitAlive(void){
	CAN_transmitByte(POWR_ALIVE, ++counter_alive);
...
extern int16_t* Interface_readController(void){
	...
	/* Face buttons */
	if (controller_package2[4] & 0x01){
		/* A */
		controller_data[7] = 1;
	}else if (controller_package2[4] & 0x02) {
		/* B */
		controller_data[7] = -1;
	}else{
		/* No face buttons pressed */
		controller_data[7] = 0;
	}
